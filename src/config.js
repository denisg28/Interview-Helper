...
function getConfigDir() {
    ...
    if (platform === 'win32') {
    // Windows: %APPDATA%\secure-app-config
    configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'secure-app-config');
    } else if (platform === 'darwin') {
    // macOS: ~/Library/Application Support/secure-app-config
    configDir = path.join(os.homedir(), 'Library', 'Application Support', 'secure-app-config');
    } else {
    // Linux and others: ~/.config/secure-app-config
    configDir = path.join(os.homedir(), '.config', 'secure-app-config');
    }
